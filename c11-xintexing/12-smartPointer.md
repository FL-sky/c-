...
- 利用 C++中一个对象出了其作用域会被自动析构，因此我们只需要在构造函数的时候申请空间，而在析构函数（在离开作用域时调用）的时候释放空间，这样，就减轻了程序员在编码过程中，考虑资源释放的问题，这就是**RAII**。
...
具体而言，C11 的 stl 中为大家带来了 3 种智能指针，正确合理的使用可以有效的帮助大家管理资源，当然，在 C++的使用智能指针没有像 Java,python 这种具备垃圾回收机制的语言那么舒适，毕竟，程序员还需要做一些额外的事情，但是，这远比传统的 C 或者 C++更加优雅。
...
在早期有一个 auto_ptr，这四种指针在使用上有区别：

- auto_ptr 有缺陷是过时的产物。

- unique_ptr 对 auto_ptr 的问题进行了修正。

- shared_ptr 使用了引用计数，但是会出现循环引用的问题需要配合后面的 weak_ptr 一起使用。
